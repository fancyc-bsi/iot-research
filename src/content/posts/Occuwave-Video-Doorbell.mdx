...
seems like we need a valid login, I tried using the account we created during the pairing of the device with no luck. 

# SSL Pinning Bypass - Mobile app

Their mobile app is called "Seeing", it's filled with external ads and pushes you to subscribe to their cloud storage.

As we proved above, regardless of if you pay for their cloud storage, the device will take occasional screenshots and store on their cloud servers.

In this section we will be attempting to intercept the coms between the mobile app and connected cloud servers or directly to the doorbell itself.
...
This device followed very poor security practices, from not checking certificates to storing photos on their cloud servers without the user knowing. This device is a prime example of why you should always wipe your devices before getting rid of them.
...
Lastly, the root password was easily cracked and the device was running an outdated version of openWRT, a root shell was obtained instantly due to no login prompt.
